...
func checkDB() {
	...
	if err != nil {
		state.mu.Lock()
		state.connected = false
		state.errMsg = fmt.Sprintf("Cannot open database connection: %v", err)
		state.lastChecked = time.Now()
		state.mu.Unlock()
		return
	}
	...
	if err := db.PingContext(ctx); err != nil {
		state.mu.Lock()
		state.connected = false
		state.errMsg = fmt.Sprintf("Cannot reach database at %s\n%v", u.Host, err)
		state.lastChecked = time.Now()
		state.mu.Unlock()
		return
	}
...
func rootHandler(w http.ResponseWriter, r *http.Request) {
	...
	data := pageData{
		...
		LastChecked: lastChecked.Format("15:04:05"),
  ...
  <div class="grid">
    <div class="card wide">
      ...
      <div class="card-value">
        {{if .Connected}}
        <span class="dot green"></span>{{.DatabaseURL}}
        {{else}}
        <span class="dot red"></span>{{.DatabaseURL}}
        {{end}}
      </div>
    </div>
    <div class="card">
